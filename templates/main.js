...
/* Content-Security-Policy friendly bootstrap.
 * Page context is passed via data-* attributes on this <script> tag (set in header.html)
 * instead of a separate inline <script>, so a stricter script-src can be used. */
(function () {
	var s = document.currentScript;
	window.resourceVersion = (s && s.getAttribute('data-resource-version')) || '';
	window.configRoot = (s && s.getAttribute('data-config-root')) || '';
	window.inMod = !!(s && s.getAttribute('data-in-mod') === 'true');
	window.modRoot = (s && s.getAttribute('data-mod-root')) || '';
	window.active_page = (s && s.getAttribute('data-active-page')) || '';
	window.thread_id = (s && s.getAttribute('data-thread')) || '';
	var b = (s && s.getAttribute('data-board')) || '';
	window.board_name = b !== '' ? b : false;
})();
...
var __vichan_ready_fired = false;
function ready() {
	// Idempotent: safe to call more than once (e.g. a stray inline call plus the
	// DOMContentLoaded trigger below).
	if (__vichan_ready_fired) {
		return;
	}
	__vichan_ready_fired = true;
	for (let i = 0; i < onready_callbacks.length; i++) {
		onready_callbacks[i]();
	}
}

/* Run ready() ourselves once the DOM is parsed, instead of relying on an inline
 * <script>ready()</script> at the bottom of every page. */
